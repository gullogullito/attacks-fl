...
#   Next step is to define our model, we'll use pytorch resnet18
def get_model(num_classes=10):

    vgg_model = vgg16(weights="DEFAULT")

    vgg_model.classifier[6] = torch.nn.Linear(in_features=4096, out_features=10)
...
def get_gradient(client_model : FlexModel, client_data: Dataset):
    ...
    test_data = client_data.to_torchvision_dataset(transform = cifar_transforms)
...
pool = FlexPool.client_server_pool(fed_dataset=get_dataset(), init_func=build_server_model)
honest_client = pool.clients.select(1)
pool.servers.map(copy_server_model_to_clients, honest_client)

#print(client_to_leak.map(get_gradient)[0])
...
def DLG(client_model : FlexPool, gradients):
    ...
    imshow(dummy_data.detach().squeeze(), 'Dummy data')
